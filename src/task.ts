...
import { verifyOnSourcify } from './sourcifyVerifier';
...
// Maps to ../bush. Every task lookup and enumeration (`getAllTaskIds`, and the hardhat tasks built on top of it, such
// as `save-action-ids`, `check-deployments` or `build-timelock-authorizer-config`) is scoped to this directory.
const VERSION_ROOTS = ['bush'].map((version) => path.resolve(__dirname, `../${version}`));

// Maps to bush/tasks, bush/deprecated, bush/scripts.
...
export default class Task {
  ...
  // constructorArguments and libs are unused: Sourcify verification matches by bytecode against the build info
  // rather than needing ABI-encoded constructor arguments, but the parameters are kept so callers built around the
  // previous Etherscan-based verifier don't all need to be updated.
  async verify(
    name: string,
    address: string,
    ...
    try {
      ...
      const url = await verifyOnSourcify(task, network, name, address, chainId);
      logger.success(`Verified contract ${name} on Sourcify at ${url}`);
    } catch (error) {
  ...
  private _parseRawInput(rawInput: RawInputKeyValue): Input {
    return Object.keys(rawInput).reduce((input: Input, key: Network | string) => {
      ...
      if (!this._isTask(item)) {
        ...
        // { 'BushHelpers': new Task('20210418-vault', TaskMode.READ_ONLY) }
        // the input value will be the output of name 'BushHelpers' of said task.
  ...
  private _findTaskId(idAlias: string): string {
    const matches = _taskIdsIn(VERSION_ROOTS).filter((taskDirName) => taskDirName.includes(idAlias));
  ...
  /**
   * Return all directories inside the top 3 fixed task directories of the enumerated roots, in a flat, sorted array.
   */
  static getAllTaskIds(): string[] {
    return _taskIdsIn(VERSION_ROOTS);
  }
}

/**
 * Return the task IDs found in the tasks, deprecated and scripts directories of the given roots.
 */
function _taskIdsIn(versionRoots: string[]): string[] {
